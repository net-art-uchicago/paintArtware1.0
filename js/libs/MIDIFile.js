      ...
      if (events[i].type === MIDIEvents.EVENT_META) {
        switch (events[i].subtype) {
          ...
          case MIDIEvents.EVENT_META_SMTPE_OFFSET:
            ...
            break
          case MIDIEvents.EVENT_META_KEY_SIGNATURE:
            if (typeof events[i].key !== 'number' || events[i].key < -7 ||
						events[i].scale > 7) {
              throw new Error('Event #' + i + ':The key signature key must be' +
							' between -7 and 7')
            }
            if (typeof events[i].scale !== 'number' ||
...
function MIDIFileTrack (buffer, start) {
  ...
  if (!buffer) {
    ...
    if (!(
      this.datas.getUint8(MIDIFileTrack.HDR_LENGTH + (trackLength - 3)) === 0xFF &&
				this.datas.getUint8(MIDIFileTrack.HDR_LENGTH + (trackLength - 2)) === 0x2F &&
				this.datas.getUint8(MIDIFileTrack.HDR_LENGTH + (trackLength - 1)) === 0x00)) {
      throw new Error('Invalid MIDIFileTrack (0x' + start.toString(16) + ') :' +
    ...
    do {
      ...
      for (i = 0, j = trackParsers.length; i < j; i++) {
        if (trackParsers[i].curEvent) {
          if (smallestDelta === -1 || trackParsers[i].curEvent.delta <
...
MIDIFile.prototype.getLyrics = function () {
  ...
  for (i = 0, j = events.length; i < j; i++) {
    ...
    if (event.subtype === MIDIEvents.EVENT_META_LYRICS) {
      ...
      } else if (String.fromCharCode.apply(String, event.data).indexOf('words') === 0) {
        texts.length = 0
