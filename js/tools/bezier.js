  ...
  events: {
    ...
    mousemove: function (e) {
      ...
      if (state.selected && state.mousePressed) {
        ...
        const start = { x: cc.width, y: cc.height }
        const cp1 = { x: cc.width + 50, y: cc.height + 100 }
        const cp2 = { x: cc.width + 200, y: cc.height + 100 }
        const end = { x: cc.width + 200, y: cc.height + 50 }
        ...
        // Control points
        ctx.beginPath()
        ctx.arc(cp1.x, cp1.y, 5, 0, 2 * Math.PI * 50 * Math.random * 0.07) // Control point one
        ctx.arc(cp2.x, cp2.y, 5, 0, 2 * Math.PI * 50 * Math.random * 0.03) // Control point two
